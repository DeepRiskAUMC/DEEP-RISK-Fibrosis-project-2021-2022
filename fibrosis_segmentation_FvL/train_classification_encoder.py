...
from models.classification_models import ClassificationHead
from data_loading.load_classification_data import load_classification_data_clinical
from models.segmentation_models import Floor_2d_Unet_encoder_only

class ClassificationSegmentationModel(pl.LightningModule):
    def __init__(self, model_name, fibrosis_checkpoint, loss_function_string, lr, prediction_task, extra_input, hidden_layers_lin=[64], train_loss_weights=None, val_loss_weights=None, use_val_weights=False, use_MRI_features=False, flatten_or_maxpool='maxpool', cross_validate=False):
        super().__init__()
        ...
        self.flatten_or_maxpool = flatten_or_maxpool
        ...
        if fib_model_name == 'Floor_2d_Unet':
            self.fib_half_model = Floor_2d_Unet_encoder_only(2, bilinear=self.bilinear, feature_multiplication=self.feature_multiplication)
            self.fib_half_model = load_pretrained_layers(self.fib_half_model, full_fib_model)

        self.myo_model.freeze()
        # we freeze the fibrosis model
        self.fib_half_model.requires_grad_(False)
        del full_fib_model

        self.segment_model_output_size = 256 * self.feature_multiplication
        ...
        if self.use_MRI_features:
            count_clin_features = 8
        else:
            count_clin_features = 0
        self.classification_head = ClassificationHead(self.segment_model_output_size*count_extra_input + count_clin_features, lin_channels=hidden_layers_lin, sigmoid_finish=self.sigmoid_finish, use_MRI_features=self.use_MRI_features, flatten_or_maxpool=self.flatten_or_maxpool)
    ...
    def forward(self, imgs, clinical):
        ...
        if '2D' in self.segment_model_name:
            ...
            fib_pred = torch.zeros((imgs.shape[0], self.segment_model_output_size, imgs.shape[2], 8, 8), dtype=input_datatype, device=device)
            for i in range(imgs.shape[2]):
                if torch.any(imgs[:,:,i,:,:] > 0):
                    LGE_slice = imgs[:,:,i,:,:]
                    myo_slice = myo_pred[:,:,i,:,:]
                    concat_slice = torch.cat([LGE_slice, myo_slice], dim=1)
                    fib_pred_slice = self.fib_half_model(concat_slice)
        ...
        output = self.classification_head(concat_input, clinical)
...
def train(args):
    ...
    if args.continue_from_path == 'None':
        ...
        model = ClassificationSegmentationModel(args.model, 
                                                args.fib_checkpoint, 
                                                args.loss_function, 
                                                args.lr, 
                                                args.prediction_task, 
                                                args.extra_input, 
                                                args.hidden_layers_lin, 
                                                ...
                                                flatten_or_maxpool=args.flatten_or_maxpool,
                                                cross_validate=args.cross_validate)
        trainer.fit(model, train_loader, val_loader)
    else:
        print(f'Continued training from checkpoint {args.continue_from_path}')
        model = ClassificationSegmentationModel.load_from_checkpoint(args.continue_from_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='encoder', type=str,
    ...
    parser.add_argument('--use_MRI_features', default='True', type=str,
                        ...
                        choices=['True', 'False'])
    parser.add_argument('--hidden_layers_lin', default=[128,64,32], nargs='+', type=int,
                        help='Number of channels for the hidden linear layers of the classifier network.')
    parser.add_argument('--flatten_or_maxpool', default='maxpool', type=str,
    ...
    parser.add_argument('--dataset', default='AUMC3D_version3', type=str,
    ...
    parser.add_argument('--fib_checkpoint', default=r'outputs\segment_logs\fibrosis\lightning_logs\version_61\checkpoints\epoch=113-step=8663.ckpt', type=str,
    ...
    print(f"Dataset: {args.dataset} | prediction task: {args.prediction_task} | model: {args.model} | flatten_or_maxpool: {args.flatten_or_maxpool} | use_MRI_features: {args.use_MRI_features} | layers_lin: {args.hidden_layers_lin} | resize: {args.resize} | normalize: {args.normalize} | augmentation: {args.transformations}")
    ...
    print(f"segmentation checkpoint: {args.fib_checkpoint}")
