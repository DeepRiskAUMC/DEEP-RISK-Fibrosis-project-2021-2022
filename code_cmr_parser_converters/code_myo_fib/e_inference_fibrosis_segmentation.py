...
import yaml
from e_datasets import process_images_3d
from e_inference_myocard_segmentation import visualize_stack
from e_preprocessing import (compose_inference_transforms_with_segmentations,
                           get_array_from_nifti, uncrop)
from tqdm import tqdm
from e_train_fib_segmentation import init_model
...
def main(args):
    ...
    nifti_files = [x for x in sorted(IN_DIR.glob(f'*{args.files_must_contain}*.{args.input_extension}'))]
    for nifti_file in tqdm(nifti_files):
        ...
        if args.pred_myo_path == None:
            ...
            if len(pred_myo_file) == 0:
                ...
                continue
            # EMMA: two myopred files, don't understand why
            # assert len(pred_myo_file) == 1, f"Multiple myocardium predictions found for sequence {nifti_file=}, {pred_myo_file=}"
            if len(pred_myo_file) > 1:
                answer = "???"
                while answer not in [0, 1, 2, 3]:
                    answer = input(f"Multiple myocardium predictions found for sequence {nifti_file=}, {pred_myo_file=}, please type the int for which file is correct.\n")
                    if answer in [0, 1, 2, 3]:
                        pred_myo_file = [pred_myo_file[answer]]
                    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--files_must_contain', type=str, default="psir",
    ...
    parser.add_argument('--input_extension', type=str, default='nii.gz',
