...
from e_preprocessing import myoseg_to_roi, normalize_image_func, normalize_image, set_image_range, roi_crop_multiple_images, get_array_from_nifti
...
def process_images_3d(images, image_names, transform, roi_crop, depth_to_batch=False):
    ...
    images = [torch.from_numpy(x.astype(np.float32)).float().view(D, 1, H, W) for x in images]
