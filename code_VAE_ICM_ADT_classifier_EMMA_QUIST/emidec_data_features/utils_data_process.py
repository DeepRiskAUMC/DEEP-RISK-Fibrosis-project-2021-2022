...
def create_patches(path, patient, cohort_name, Train):
    ...
    binMask_path = "../myo_lv/binMask/"
    if not os.path.exists(binMask_path):
        os.makedirs(binMask_path)    
...
def preprocess_files(path_training, minf_cohort, cohort_name, Train = True):
    ...
    save_path = "../processed_emidec_dataset/whole_image/Training_cropped"
...
def load_preprocessed_files(path_training, minf_cohort, cohort_name):
    ...
    save_path =  os.path.join(path_training, "whole_image/Training_cropped")
    for j in range(len(minf_cohort)):
        path_files = os.path.join(path_training, minf_cohort[j] )
        patient_id = minf_cohort[j]
...
def radiomics_feature_extractor(img_dir, mask_dir, label):
    ...
    for i in range(len(img_dir)):
        ...
        # Enable "Joint Average" and disable "Sum Average" for GLCM
        radiomics_extractor.enableFeatureClassByName('glcm', 'Joint Average')
        # radiomics_extractor.enableFeatureClassByName('glcm', 'Sum Average', enabled=False)
        ...
        radiomics_["Patient_ID"] = fn  # Add patient ID as a column

        radiomicsfeats_all = pd.concat([radiomicsfeats_all, radiomics_], ignore_index=True)
...
def load_clinical_info(path, info_files, emidec_columns, folders):
    ...
    for i in range(len(info_files)):
        ...
        with open(Info_path, 'r', encoding='mac_roman') as Info:
            ...
            if len(lines) >= len(emidec_columns) + 2:
                for j in range(2, len(emidec_columns) + 2):
                    value = lines[j].rstrip().split(":")[1][:3]
                    clinical_info.iloc[i, j - 2] = value
