...
#
def network_factory(cfg, **kwargs):
    if cfg.BACKBONE == "drnd":
        print("Backbone: DRN-D")
        backbone = DRN
    elif cfg.BACKBONE == "resnet38":
    ...
    class BaselineCAM(backbone):
        ...
        def __init__(self, config, pre_weights=None, num_classes=2, dropout=True):
            super().__init__()
    ...
    class SoftMaxAE(backbone):
        ...
        def __init__(self, config, pre_weights=None, num_classes=21, dropout=True,
                    use_aspp=False, use_focal=False, **kwargs):
        ...
        def _init_aspp(self):
            self.aspp = ASPP(self.fan_out(), self.fan_out_dict()['conv3'], 8, self.NormLayer)
        ...
        def _init_decoder(self, num_classes):
            ...
            dim_conv3 = self.fan_out_dict()['conv3']
            # pre-processing for shallow features
            self.shallow_mask = GCI(self.NormLayer, inplanes=dim_conv3)
            self.from_scratch_layers += self.shallow_mask.from_scratch_layers

            # Stochastic Gate
            self.sg = StochasticGate()
            self.fc8_skip = nn.Sequential(conv2d(dim_conv3, 48, 1, bias=False), bnorm(48), nn.ReLU())
            self.fc8_x = nn.Sequential(conv2d(dim_conv3+48, dim_conv3, kernel_size=3, stride=1, padding=1, bias=False),
                                       bnorm(dim_conv3), nn.ReLU())

            # decoder
            #CHANGED removed dropout
            self.last_conv = nn.Sequential(conv2d(dim_conv3, dim_conv3, kernel_size=3, stride=1, padding=1, bias=False),
                                           bnorm(dim_conv3), nn.ReLU(), #nn.Dropout(0.5),
                                           conv2d(dim_conv3, dim_conv3, kernel_size=3, stride=1, padding=1, bias=False),
                                           bnorm(dim_conv3), nn.ReLU(), #nn.Dropout(0.1),
                                           conv2d(dim_conv3, num_classes - 1, kernel_size=1, stride=1))
