...
from .criterions import Diceloss, smoothed_dice_score, get_accuracy, UnderfittingDiceloss
from .segmentation_models import Floor_3D_half_Unet, Simple_2d_Unet, CA_2d_Unet, Floor_3D_full_Unet, Floor_2d_Unet
...
class MyDice(torchmetrics.Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        if self.smoothing == 0:
            non_empty =  (target.amax(dim=non_batch_dims, keepdim=False) != 0) # * (preds.amax(dim=(1, 2, 3), keepdim=False) != 0)
            #non_empty = (target.amax(dim=(1, 2, 3), keepdim=False) != 0)
            if non_empty.sum() == 0:
                return
...
class SegmentationModel(pl.LightningModule):
    ...
    def train(self, mode=True):
        """Override train to freeze layers"""
    ...
    def log_images(self, split):
        ...
        # optional depth dimension to batch dimension for visualization
