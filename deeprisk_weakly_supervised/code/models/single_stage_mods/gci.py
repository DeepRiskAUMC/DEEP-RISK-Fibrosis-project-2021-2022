...
class GCI(nn.Module):
    ...
    def __init__(self, NormLayer=nn.BatchNorm2d, inplanes=256):
        super(GCI, self).__init__()

        self.NormLayer = NormLayer
        self.from_scratch_layers = []

        self._init_params(inplanes)
    ...
    def _init_params(self, inplanes):

        self.fc_deep = nn.Sequential(self._conv2d(inplanes, inplanes*2, 1, bias=False), \
                                     self._bnorm(inplanes*2), nn.ReLU())

        self.fc_skip = nn.Sequential(self._conv2d(inplanes, inplanes, 1, bias=False), \
                                     self._bnorm(inplanes, affine=False))

        self.fc_cls = nn.Sequential(self._conv2d(inplanes, inplanes, 1, bias=False), \
                                    self._bnorm(inplanes), nn.ReLU())
