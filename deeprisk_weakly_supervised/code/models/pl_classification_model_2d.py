...
class ModelBase(pl.LightningModule):
    ...
    def forward(self, x, myo_seg=None):
        ...
        x = self.classifier(x) # outputs Class Activation Map
    ...
    def myo_seg_to_mask(self, x, dilation=0, size="cam"):
        ...
        # If model is trained with a mix of masked and unmasked inputs,
        # don't mask during evaluation.
        # If model is exclusively trained with masked inputs,
        # also mask during evaluation .
        myo_mask_prob = self.myo_mask_prob if (self.training or self.myo_mask_prob == 1) else 0
        no_mask = torch.rand(x.shape[0]) >  myo_mask_prob # sample probs for each batch item
        x[no_mask, ...] = 1.0 # everywhere 1 is equivalent to no masking
...
class NormalizedGlobalWeightedPooling(nn.Module):
    """ nGWP as described in https://arxiv.org/pdf/2005.08104.pdf, following implementation https://github.com/visinf/1-stage-wseg/blob/master/models/stage_net.py
        ...
        Warning: There might be a mistake somewhere, this didn't really work.
    """
...
class MaskedAdaptiveAvgPool2d(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        """ Empty masks will have value zero -> prediction 0.5
            Probably not good (when using good myocardium predictions),
            since no myocardium means no fibrosis (i.e. prediction should really be 0).
            However, not relevant for training, since gradients don't flow through."""
        return x
